...
function BoxNode({ data }: { data: any }) {
  const { icon, title, subtitle, dbs = [], techs = [], handles = { in: Array.from(ALL_SIDES), out: Array.from(ALL_SIDES) }, theme = "flow", sections = [], cardWidth } = data || {};
  const t = (palette as any)[theme] || (palette as any).flow;
  const inHandles = Array.isArray(handles.in) ? handles.in : Array.from(ALL_SIDES);
  const outHandles = Array.isArray(handles.out) ? handles.out : Array.from(ALL_SIDES);
  const width = typeof cardWidth === "number" ? cardWidth : 260;
  const sectionsCols = width >= 340 && Array.isArray(sections) && sections.length > 1 ? 2 : 1;
  return (
    <div style={{ ...cardBase, background: t.bg, borderColor: t.border, width }}>
      <div style={row}>
        <div style={{ ...iconWrapBase, background: t.iconBg }}>{icon}</div>
        <div style={{ minWidth: 0 }}>
          <div style={titleStyle}>{title}</div>
          {subtitle && <div style={subtitleStyle}>{subtitle}</div>}
          {dbs.length > 0 && (
            <div style={badgeRow}>{dbs.map((name: string) => { const s = getDbStyle(name); return (<span key={String(name)} style={{ ...badgeBase, background: s.bg, borderColor: s.border, color: s.text }}><Database size={12} /> {name}</span>); })}</div>
          )}
          {techs.length > 0 && (
            <div style={badgeRow}>{techs.map((name: string) => (<span key={String(name)} style={{ ...badgeBase, background: t.pillBg, borderColor: t.pillBorder }}><Code2 size={12} /> {name}</span>))}</div>
          )}
        </div>
        <span className="pulse-dot" style={{ position: "absolute", right: 8, top: 8, width: 8, height: 8, borderRadius: 8, background: "#22c55e" }} />
      </div>
      {Array.isArray(sections) && sections.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: sectionsCols === 2 ? "1fr 1fr" : "1fr", gap: 4, marginTop: 6 }}>
          {sections.map((s: any, idx: number) => (
            <div key={idx} style={{ border: `1px solid ${t.pillBorder}`, background: t.pillBg, borderRadius: 8, padding: 6, wordBreak: "break-word" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 4, fontWeight: 600 }}>
                <span style={{ ...iconWrapBase, background: "rgba(0,0,0,.05)", boxShadow: "none", padding: 4 }}>{s.icon}</span>
                <span style={{ fontSize: 11 }}>{s.title}</span>
              </div>
              {s.subtitle && <div style={{ fontSize: 10, color: "#475569", marginTop: 2 }}>{s.subtitle}</div>}
            </div>
          ))}
        </div>
      )}
      {/* Renderizar todos los handles definidos en el JSON, incluyendo -1, -2, etc. */}
...
const addIconsToNodes = (nodes: any[]) => {
  const iconMap: Record<string, any> = {
    ...
    incomingmail: <Server size={24} />,
    ticketmailmanager: <Clock3 size={24} />
  };
...
function TextNode({ id, data }: { id: string; data: any }) {
  ...
  // Mantén sincronizado el contenido con data.value
  useEffect(() => {
    if (ref.current && ref.current.innerText !== (data?.value ?? "")) {
      ref.current.innerText = data?.value ?? "";
    }
  }, [data?.value]);

  const commit = () => {
    if (ref.current) data?.onChange?.(id, ref.current.innerText);
  };

  const stop = (e: React.SyntheticEvent) => {
    // evita que React Flow capture eventos de edición
    e.stopPropagation();
  };

  return (
    <div style={{ background: "transparent", border: "none", padding: 0 }}>
      {/* Handle para arrastrar el nodo sin interferir con la edición */}
      <div
      ...
      {/* Área editable: sin fondo, sin borde */}
      <div
        ref={ref}
        contentEditable
      ...
      {/* Opcional: handles para conectar */}
      <Handle id="in-left" type="target" position={Position.Left} style={{ width: 8, height: 8, background: "#6366f1" }} />
      <Handle id="out-right" type="source" position={Position.Right} style={{ width: 8, height: 8, background: "#6366f1" }} />
    </div>
  );
}
...
function ResizableSquareNode() {
  ...
  return (
    <div style={{
      ...
      <NodeResizer minWidth={80} minHeight={80} lineStyle={{ borderColor: '#6366f1' }} handleStyle={{ width: 8, height: 8, borderRadius: 2 }} />
      {/* Conectores en los 4 lados */}
      <Handle id="in-top" type="target" position={Position.Top} style={{ width: 8, height: 8, background: "#6366f1" }} />
      <Handle id="out-top" type="source" position={Position.Top} style={{ width: 8, height: 8, background: "#6366f1", left: 'calc(50% + 12px)' }} />
      <Handle id="in-right" type="target" position={Position.Right} style={{ width: 8, height: 8, background: "#6366f1" }} />
      <Handle id="out-right" type="source" position={Position.Right} style={{ width: 8, height: 8, background: "#6366f1", top: 'calc(50% + 12px)' }} />
      <Handle id="in-bottom" type="target" position={Position.Bottom} style={{ width: 8, height: 8, background: "#6366f1" }} />
      <Handle id="out-bottom" type="source" position={Position.Bottom} style={{ width: 8, height: 8, background: "#6366f1", left: 'calc(50% + 12px)' }} />
      <Handle id="in-left" type="target" position={Position.Left} style={{ width: 8, height: 8, background: "#6366f1" }} />
      <Handle id="out-left" type="source" position={Position.Left} style={{ width: 8, height: 8, background: "#6366f1", top: 'calc(50% + 12px)' }} />
    </div>
  );
}
      ...
      <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 12, fontSize: 12, color: "#475569" }}>
        ...
        <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 12, background: "#f1f5f9" }}>
          ...
          <ul style={{ paddingLeft: 18, marginTop: 6 }}>
            ...
            <li>MS ticket-hook-service: .NET 10</li>
            <li>MS ticket-out-service: .NET 10</li>
        ...
        <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 12, background: "#f1f5f9" }}>
          ...
          <ul style={{ paddingLeft: 18, marginTop: 6 }}>
            <li><b>event_message_received</b>: Producer = MS ticket-hook-service · Consumer = MS ticket-intake-service · <b>partitions</b> = 60</li>
...
function ArquitecturaReactFlow() {
  ...
  const resetLayout = () => {
    setNodes(addIconsToNodes(arquitecturaData.nodes)); setEdges(arquitecturaData.edges.map(edge => ({
      ...edge,
      animated,
      markerEnd: { ...edge.markerEnd, type: MarkerType.ArrowClosed }
    })));
  ...
  const addResizableSquare = useCallback(() => {
    ...
    setNodes((nds) => [
      ...nds,
      {
        ...
        data: {},
        // tamaño inicial (requerido para que el resizer tenga base)
  ...
  const onTextChange = useCallback((id: string, value: string) => {
    setNodes(ns =>
      ns.map(n => (n.id === id ? { ...n, data: { ...n.data, value } } : n))
    );
  }, [setNodes]);
  ...
  const addText = useCallback(() => {
    ...
    setNodes(ns => [
      ...ns,
      {
        ...
        data: { value: 'Nuevo texto', onChange: onTextChange },
        style: { width: 260, height: 120 },
        dragHandle: ".drag-handle",        // <- importantísimo
      }
    ]);
      ...
      <div style={{ width: "95%", padding: "16px", margin: "0 auto", boxSizing: "border-box" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={addResizableSquare}
              style={{ borderRadius: 16, border: "1px solid #e2e8f0", padding: "8px 12px", fontSize: 13, background: "#fff", color: "#0f172a", cursor: "pointer" }}>
              Agregar cuadrado
            </button>
            <button onClick={addText} style={{ borderRadius: 16, border: "1px solid #e2e8f0", padding: "8px 12px", fontSize: 13, background: "#fff", color: "#0f172a", cursor: "pointer" }}>
              + Texto
            </button>
